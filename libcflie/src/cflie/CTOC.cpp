...
bool CTOC::startLogging(std::string strName, std::string strBlockName) {
  ...
  if(bFound) {
    ...
    if(bFound) {
      char cPayload[5] = {0x01, static_cast<char>(lbCurrent.nID), static_cast<char>(teCurrent.nType), static_cast<char>(teCurrent.nID)};
...
bool CTOC::registerLoggingBlock(std::string strName, double dFrequency) {
  ...
  if(dFrequency > 0) { // Only do it if a valid frequency > 0 is given
    ...
    char cPayload[4] = {0x00, static_cast<char>(nID), static_cast<char>(d10thOfMS)};
...
bool CTOC::enableLogging(std::string strBlockName) {
  ...
  if(bFound) {
    ...
    char cPayload[3] = {0x03, static_cast<char>(lbCurrent.nID), static_cast<char>(d10thOfMS)};
...
bool CTOC::unregisterLoggingBlockID(int nID) {
  char cPayload[2] = {0x02, static_cast<char>(nID)};
